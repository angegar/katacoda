...
#
# Ask for GitHub user and PAT
#
echo -e "${COLQUESTION}Please enter your ${GITHUB} user name (dxc shortname):${COLRESET}"
read SHORTNAME
export SHORTNAME
...
echo -e "${COLLOGS}"
sed -i "s/github_url/${GITHUB}\/${SHORTNAME}\/pet-clinic-stripe2/g" ~/nginx/index.html
...
echo -e "${COLLOGS}"
curl ${CURL_NODEBUG} -H "Authorization: token $TOKEN" -X GET https://${GITHUB}/api/v3/repos/$SHORTNAME/pet-clinic-stripe2/hooks | grep id | cut -d ":" -f2 | cut -c 1-6> ids.txt
...
do
    name="$line"
        curl ${CURL_NODEBUG} -H "Authorization: token $TOKEN" -X DELETE https://${GITHUB}/api/v3/repos/$SHORTNAME/pet-clinic-stripe2/hooks/"$line"
done < "$filename"
}
remove_existing_webhook
...
# Bring up Arifactory
#
bringing_up_artifactory()
{
echo -e "${COLINFO}Bringing up Artifactory...${COLRESET}"
echo -e "${COLLOGS}"
...
echo -e "${COLLOGS}"
docker pull devopsdojo/jenkins-yb-s2:latest
docker run --name jenkins-yb-s2 -d -p 8080:8080 -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/shortname.txt:/tmp/shortname.txt devopsdojo/jenkins-yb-s2:latest
}
bringing_up_jenkins
...
curl ${CURL_NODEBUG} -H "Authorization: token $TOKEN" -X POST --data '{"name": "web","active": true,"events": [ "push", "pull_request" ], "config":{"url": "https://'"$JenkinsUrl"'/github-webhook/","content_type":"json"}}' https://${GITHUB}/api/v3/repos/$SHORTNAME/pet-clinic-stripe2/hooks
}
adding_webhook
